        ...
        parameters: {
            ...
            properties: {
                action: {
                    ...
                    description:
                        'Hành động cần thực hiện. find_and_highlight: làm sáng khung chat. find_and_replace: thay thế chữ. clear_highlight: Xóa toàn bộ highlight hiện tại.',
                },
                query: {
                    type: 'string',
                ...
                replacement: {
                    type: 'string',
                    description:
                        'Chuỗi thay thế (chỉ dùng khi action = find_and_replace). Mặc định là chuỗi rỗng nếu không truyền.',
                },
                is_regex: {
                    type: 'boolean',
                    description:
                        'Set thành true nếu query là một biểu thức Regex. Mặc định là false (tìm chuỗi chính xác).',
                },
                whole_word: {
                    type: 'boolean',
                ...
                dry_run: {
                    type: 'boolean',
                    description:
                        'Nếu true (chỉ dùng cho find_and_replace), sẽ CHỈ trả về danh sách các thay đổi dự kiến mà KHÔNG thực sự lưu thay đổi. Rất hữu ích để xem trước kết quả. Mặc định false.',
                },
            },
            required: ['action'],
        },
    ...
    execute: async (args: Record<string, any>, context: { adapter: SillyTavernAdapter }): Promise<ToolResult> => {
        ...
        try {
            if (action === 'clear_highlight') {
                ...
                const result = await context.adapter.findAndReplace(
                    query,
                    replacement,
                    isRegex,
                    caseInsensitive,
