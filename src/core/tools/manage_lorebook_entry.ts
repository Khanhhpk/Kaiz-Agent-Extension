...
export const manageLorebookEntryTool: ITool = {
        ...
        parameters: {
            ...
            properties: {
                ...
                keys: {
                    ...
                    items: { type: 'string' },
                    description:
                        '(Tuỳ chọn) Danh sách các từ khóa kích hoạt entry này. Ví dụ: ["apple", "banana"]. (Dùng cho create/edit)',
                },
                content: {
                    type: 'string',
                ...
                constant: {
                    type: 'boolean',
                    description:
                        '(Tuỳ chọn) Đặt thành true nếu muốn entry luôn luôn được kích hoạt bất chấp từ khóa. (Dùng cho create/edit)',
                },
                disable: {
                    type: 'boolean',
    ...
    execute: async (args: Record<string, any>, context: { adapter: SillyTavernAdapter }): Promise<ToolResult> => {
        ...
        if ((args.action === 'edit' || args.action === 'delete') && (args.uid === undefined || args.uid === null)) {
            return {
                content:
                    "[LỖI] Thiếu tham số 'uid'. Bạn bắt buộc phải cung cấp UID của entry nếu muốn edit hoặc delete.",
                isError: true,
            };
        }
