...
export class PresetGitModal {
    ...
    private async renderStorageMetrics(): Promise<void> {
        ...
        $('#kaiz-pg-stat-commits')
            ...
            .attr(
                'title',
                `Preset này có ${currentPresetStats.commits} bản lưu (Toàn hệ thống: ${stats.totalCommits} bản lưu)`,
            );
    ...
    private async renderStagingSandbox(): Promise<void> {
        ...
        if (isDirty) {
            ...
            $('#kaiz-pg-staging-summary').html(
                `<b>${diff.totalChanges} thay đổi chưa lưu:</b> +${diff.added} tạo mới, ~${diff.modified} sửa, -${diff.deleted} xóa. <span style="opacity: 0.8">${sourceText}</span>`,
            );
