...
export class UICustomizationModal {
    ...
    private async renderSnapshots() {
        ...
        if (snapshots.length === 0) {
            list.append(
                '<div style="color: #aaa; font-style: italic; text-align: center; padding: 20px;">Chưa có lịch sử thay đổi giao diện nào.</div>',
            );
            return;
        }
    ...
    private async renderThemeLibrary() {
        ...
        if (themes.length === 0) {
            list.append(
                '<div style="color: #aaa; font-style: italic; text-align: center; padding: 20px;">Thư viện trống. Bấm "Khôi phục Default" để nạp lại theme mẫu.</div>',
            );
            return;
        }
